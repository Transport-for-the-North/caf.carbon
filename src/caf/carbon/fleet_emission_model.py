...
class FleetEmissionsModel:
    ...
    def __init__(self, parameters):
        ...
        region_filter = region_filter[region_filter["stb_name"].isin(parameters.regions)]

        # %% Scenario Agnostic
        if parameters.fleet_year == 2018:
            index_fleet = scenario_invariant_2018.IndexFleet(parameters)
            invariant_data = scenario_invariant_2018.Invariant(index_fleet, parameters)
        else:
            index_fleet = scenario_invariant.IndexFleet(parameters)
            invariant_data = scenario_invariant.Invariant(index_fleet, parameters)
