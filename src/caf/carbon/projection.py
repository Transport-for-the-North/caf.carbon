...
class Model:
    ...
    def __create_future_fleet(self):
        ...
        seg_fuel_list = self.scenario.fuel_share_of_year_seg_sales[["segment", "fuel"]].drop_duplicates()
        ...
        future_fleet = future_fleet[["segment", "zone", "vehicle_type", "cohort"]].drop_duplicates()
        # Map the fuel-segments to the zones known to buy them
        self.scenario.future_fleet = future_fleet.merge(seg_fuel_list, how="left", on="segment")
    ...
    def __predict_sales(self):
        ...
        index_sales["seg_share"] = index_sales["tally"] / index_sales.groupby(["segment"])["tally"].transform("sum")
        ...
        future_sales = future_sales.merge(self.scenario.seg_share_of_year_type_sales, how="left", on=["segment"])
    ...
    def __jump_forward(fleet_df, scrappage_df, fleet_sales_df, tally_df):
        ...
        def inject_cohort(fleet_df, fleet_sales_df, tally_df, current_year):
            ...
            required_sales["deficit"] = required_sales["fleet_forecast"] - required_sales["tally"]
            ...
            fleet_df = fleet_df._append(new_cohort[["fuel", "segment", "cohort", "zone", "vehicle_type", "tally"]])
            return fleet_df
    ...
    def __project_fleet(self):
        ...
        fleet_df = self.invariant.index_fleet.fleet[["fuel", "segment", "zone", "tally", "vehicle_type", "cohort"]]
    ...
    def predict_emissions(self):
        ...
        fleet_df = fleet_df.merge(se_curve, how="left",
        ...
        # CO2 = CO2/km * km
        fleet_df["tailpipe_gco2"] = fleet_df["gco2/km"] * fleet_df["index_carbon_reduction"] * fleet_df["chainage"] * \
                                    fleet_df["biocorrection"] * fleet_df["ghg_factor"]

        # Apply reduction factors to relevant vehicles using evidence from the CCC 6th carbon budget
        # Only apply them from 2025
        fleet_df = fleet_df.merge(self.scenario.co2_reductions, on=["vehicle_type", "segment"], how="left")
        ...
        fleet_df = fleet_df.groupby(["fuel", "segment", "zone", "vehicle_type", "cohort", "year", "tally"])[
            ["tailpipe_gco2", "chainage"]].sum().reset_index()
        fleet_df = fleet_df.merge(self.invariant.grid_consumption,
                                  on=["cohort", "vehicle_type", "segment", "fuel"],
                                  how="left").fillna(0)
        fleet_df["kwh_consumption"] = fleet_df["kwh_km"] * fleet_df["chainage"]
        fleet_df = fleet_df.merge(self.invariant.grid_intensity,
                                  on="year",
                                  how="left")
        fleet_df["grid_gco2"] = fleet_df["kwh_consumption"] * fleet_df["gco2_kwh"]
        fleet_df = fleet_df.groupby(["fuel", "segment", "zone", "vehicle_type", "cohort", "year", "tally"])[
            ["tailpipe_gco2", "grid_gco2", "chainage"]].sum().reset_index()

        self.projected_fleet = fleet_df
        merged_emissions = fleet_df.groupby(["vehicle_type", "year"])[
            ["tailpipe_gco2", "grid_gco2", "chainage"]].sum().reset_index()
