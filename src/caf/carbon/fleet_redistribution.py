...
class Redistribution:
    ...
    def __input_weights(self):
        ...
        for criterion in deselection_criteria:
            traveller_types.loc[(traveller_types[criterion[0]] == criterion[1]), "selection"] = 0
        ...
        base_pop = base_pop[["zone", "soc", "people"]].pivot(values='people', index=['zone'], columns=['soc'])
        ...
        fleet_df.loc[(fleet_df["fuel"].isin(["diesel", "petrol", "petrol hybrid"])), "fuel"] = "ice"
        fleet_df = fleet_df[["fuel", "zone", "tally"]].groupby(["fuel", "zone"], as_index=False).sum()
    ...
    def __redistribution_parameters(self):
        ...
        parameters = pd.DataFrame({'par': ['soc 1', 'soc 2', 'soc 3'],
            'par val A': [-1, -1, -1], 'step 1 A': [0, 0, 0], 'step 2 A': [0, 0, 0], 'step 3 A': [0, 0, 0],
            'par val B': [-1, -1, -1], 'step 1 B': [0, 0, 0], 'step 2 B': [0, 0, 0], 'step 3 B': [0, 0, 0],
            'par val C': [-1, -1, -1], 'step 1 C': [0, 0, 0], 'step 2 C': [0, 0, 0], 'step 3 C': [0, 0, 0]})
            ...
            for par in [0, 1, 2]:
                for i in [1, 0.75, 0.5, 0.25, 0.1, 0.05, -0.05, -0.1, -0.25, -0.5, -0.75, -1]:
                    ...
                    if self.calibration_data["product iterative"].sum() < self.calibration_data["product initial"].sum():
                        initial_parameters.loc[(initial_parameters.index == par), par_val] = parameters.loc[par][par_val]
                        self.calibration_data["product initial"] = self.calibration_data["product iterative"]
        ...
        while int(parameters["step 3 A"].sum() + parameters["step 3 B"].sum() + parameters["step 3 C"].sum()) != 9:
            for par in [0, 1, 2]:
                for par_val in ["A", "B", "C"]:
                    if parameters.loc[par]["step 1 " + par_val] == 0:
                        ...
                        # try step up
                        parameters.loc[(parameters.index == par), "par val " + par_val] = parameters.loc[par][
                            "par val " + par_val] - (10 * step_size)
                        self.func(parameters)
                        if self.calibration_data["product iterative"].sum() < self.calibration_data[
                                "product initial"].sum():
                            self.calibration_data["product initial"] = self.calibration_data["product iterative"]
                            ...
                            # try step down
                            parameters.loc[(parameters.index == par), "par val " + par_val] = parameters.loc[par][
                                "par val " + par_val] + (2 * 10 * step_size)
                            self.func(parameters)
                            if self.calibration_data["product iterative"].sum() <= self.calibration_data[
                                    "product initial"].sum():
                                self.calibration_data["product initial"] = self.calibration_data["product iterative"]
    ...
    def __transform_fleet(self):
        ...
        ev_fleet.loc[(ev_fleet["fuel"].isin(["diesel", "petrol", "petrol hybrid"])), "fuel"] = "ice"
        ev_fleet = ev_fleet[["fuel", "zone", "tally", "year"]].groupby(["fuel", "zone", "year"], as_index=False).sum()
        ...
        self.projected_fleet = self.projected_fleet.merge(self.calibration_data, how="left", on="zone")
        self.projected_fleet = self.projected_fleet.merge(ev_fleet, how="left", on=["zone", "year"])
