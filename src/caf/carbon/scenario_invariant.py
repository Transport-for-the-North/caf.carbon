...
class CurveFitting:
    ...
    def generate_scrappage(invariant_obj):
        ...
        # e.g. cya = 17 is in the 15-20 group, so it's 3rd in the group, 3 from the end
...
class IndexFleet:
    ...
    def __basic_clean(self):
        ...
        fleet_archive.loc[
            (fleet_archive["vehicle_type"] == "lgv")
            & (fleet_archive["fuel"] == "petrol hybrid"),
            "fuel",
        ] = "phev"
...
class Invariant:
    ...
    def __import_shared_tables(self):
        ...
        if self.time_period:
            ...
            car_sc05_baseline_demand_am = pd.read_csv(
                str(DEMAND_PATH) + f"/sc05/vkm_by_speed_and_type_{self.index_year}_car_AM.csv"
            )
            car_sc05_baseline_demand_ip = pd.read_csv(
                str(DEMAND_PATH) + f"/sc05/vkm_by_speed_and_type_{self.index_year}_car_IP.csv"
            )
            car_sc05_baseline_demand_pm = pd.read_csv(
                str(DEMAND_PATH) + f"/sc05/vkm_by_speed_and_type_{self.index_year}_car_PM.csv"
            )
            car_sc05_baseline_demand = pd.concat(
                [
                    car_sc05_baseline_demand_am,
                    car_sc05_baseline_demand_ip,
                    car_sc05_baseline_demand_pm,
                ]
            )
            car_sc05_baseline_demand["year"] = self.index_year
            car_sc05_baseline_demand = car_sc05_baseline_demand.loc[
                car_sc05_baseline_demand.road_type == "Motorway", "total_vehicle_km"
            ].sum()
            for scenario_code in ["SC02", "SC03", "SC04", "SC05", "SC06"]:
                ...
                car_scenario_baseline_demand_am = pd.read_csv(
                    path + "/vkm_by_speed_and_type_2023_car_AM.csv"
                )
                car_scenario_baseline_demand_ip = pd.read_csv(
                    path + "/vkm_by_speed_and_type_2023_car_IP.csv"
                )
                car_scenario_baseline_demand_pm = pd.read_csv(
                    path + "/vkm_by_speed_and_type_2023_car_PM.csv"
                )
                ...
                if np.round(car_sc05_baseline_demand) != np.round(
                    car_scenario_baseline_demand
                ):
                    print("\n****!! Car Baseline demand varies across scenarios!!***\n")
            # gv demand equivalents
            hgv_sc05_baseline_demand_am = pd.read_csv(
                str(DEMAND_PATH) + f"/sc05/vkm_by_speed_and_type_{self.index_year}_hgv_AM.csv"
            )
            hgv_sc05_baseline_demand_ip = pd.read_csv(
                str(DEMAND_PATH) + f"/sc05/vkm_by_speed_and_type_{self.index_year}_hgv_IP.csv"
            )
            hgv_sc05_baseline_demand_pm = pd.read_csv(
                str(DEMAND_PATH) + f"/sc05/vkm_by_speed_and_type_{self.index_year}_hgv_PM.csv"
            )
            hgv_sc05_baseline_demand = pd.concat(
                [
                    hgv_sc05_baseline_demand_am,
                    hgv_sc05_baseline_demand_ip,
                    hgv_sc05_baseline_demand_pm,
                ]
            )
            hgv_sc05_baseline_demand["year"] = self.index_year
            hgv_sc05_baseline_demand = hgv_sc05_baseline_demand.loc[
                hgv_sc05_baseline_demand.road_type == "Motorway", "total_vehicle_km"
            ].sum()
            lgv_sc05_baseline_demand_am = pd.read_csv(
                str(DEMAND_PATH) + f"/sc05/vkm_by_speed_and_type_{self.index_year}_lgv_AM.csv"
            )
            lgv_sc05_baseline_demand_ip = pd.read_csv(
                str(DEMAND_PATH) + f"/sc05/vkm_by_speed_and_type_{self.index_year}_lgv_IP.csv"
            )
            lgv_sc05_baseline_demand_pm = pd.read_csv(
                str(DEMAND_PATH) + f"/sc05/vkm_by_speed_and_type_{self.index_year}_lgv_PM.csv"
            )
            lgv_sc05_baseline_demand = pd.concat(
                [
                    lgv_sc05_baseline_demand_am,
                    lgv_sc05_baseline_demand_ip,
                    lgv_sc05_baseline_demand_pm,
                ]
            )
            lgv_sc05_baseline_demand["year"] = self.index_year
            lgv_sc05_baseline_demand = lgv_sc05_baseline_demand.loc[
                lgv_sc05_baseline_demand.road_type == "Motorway", "total_vehicle_km"
            ].sum()
            for scenario_code in ["SC06"]:
                # hgv placeholder
                path = str(DEMAND_PATH) + f"/{scenario_code}/"
                hgv_scenario_baseline_demand_am = pd.read_csv(
                    path + "vkm_by_speed_and_type_2023_hgv_AM.csv"
                )
                hgv_scenario_baseline_demand_ip = pd.read_csv(
                    path + "vkm_by_speed_and_type_2023_hgv_IP.csv"
                )
                hgv_scenario_baseline_demand_pm = pd.read_csv(
                    path + "vkm_by_speed_and_type_2023_hgv_PM.csv"
                )
                ...
                if np.round(hgv_sc05_baseline_demand) != np.round(
                ...
                lgv_scenario_baseline_demand_am = pd.read_csv(
                    path + "vkm_by_speed_and_type_2023_lgv_AM.csv"
                )
                lgv_scenario_baseline_demand_ip = pd.read_csv(
                    path + "vkm_by_speed_and_type_2023_lgv_IP.csv"
                )
                lgv_scenario_baseline_demand_pm = pd.read_csv(
                    path + "vkm_by_speed_and_type_2023_lgv_PM.csv"
                )
                ...
                if np.round(lgv_sc05_baseline_demand) != np.round(
        ...
        if not self.time_period:
            # Check that baseline demand is the same across scenarios
            car_sc05_baseline_demand = pd.read_csv(
                str(DEMAND_PATH) + f"/sc05/vkm_by_speed_and_type_{self.index_year}_car.csv"
            )
            car_sc05_baseline_demand = car_sc05_baseline_demand.loc[
                car_sc05_baseline_demand.road_type == "Motorway", "total_vehicle_km"
            ].sum()
            for scenario_code in ["SC06"]:
                ...
                if np.round(car_sc05_baseline_demand) != np.round(
                    ...
                    print("\n****!! Car Baseline demand varies across scenarios!!***\n")
            # gv demand equivalents
            hgv_sc05_baseline_demand = pd.read_csv(
                str(DEMAND_PATH) + f"/sc05/vkm_by_speed_and_type_{self.index_year}_hgv.csv"
            )
            hgv_sc05_baseline_demand = hgv_sc05_baseline_demand.loc[
                hgv_sc05_baseline_demand.road_type == "Motorway", "total_vehicle_km"
            ].sum()
            lgv_sc05_baseline_demand = pd.read_csv(
                str(DEMAND_PATH) + f"/sc05/vkm_by_speed_and_type_{self.index_year}_lgv.csv"
            )
            lgv_sc05_baseline_demand = lgv_sc05_baseline_demand.loc[
                lgv_sc05_baseline_demand.road_type == "Motorway", "total_vehicle_km"
            ].sum()
            for scenario_code in ["SC06"]:
                ...
                hgv_scenario_baseline_demand = pd.read_csv(
                    path + "vkm_by_speed_and_type_2023_hgv.csv"
                )
                hgv_scenario_baseline_demand = hgv_scenario_baseline_demand.loc[
                    hgv_scenario_baseline_demand.road_type == "Motorway", "total_vehicle_km"
                ].sum()
                ...
                lgv_scenario_baseline_demand = pd.read_csv(
                    path + "vkm_by_speed_and_type_2023_lgv.csv"
                )
                lgv_scenario_baseline_demand = lgv_scenario_baseline_demand.loc[
                    lgv_scenario_baseline_demand.road_type == "Motorway", "total_vehicle_km"
                ].sum()
