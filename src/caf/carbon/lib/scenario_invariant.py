...
class Imputation:
    ...
    def fill_with_mean(fleet_df, key_attribute):
        ...
        fleet_df[key_attribute] = fleet_df[key_attribute].fillna(fleet_df[key_attribute_imputed])
        fleet_df.loc[(fleet_df[key_attribute].isnull()) & (fleet_df["fuel"] == "bev"), key_attribute] = 0
    ...
    def fill_with_mice(fleet_df):
        ...
        impute_matrix = IterativeImputer(random_state=0).fit_transform(impute_df)
        impute_df[:] = impute_matrix
        # Integrate values back into the dataframe
        fleet_df[["avg_co2", "avg_mass", "avg_es"]] = impute_df[["avg_co2", "avg_mass", "avg_es"]].astype("float")
        return fleet_df
        
    def segment_by_mass(fleet_df, vehicle_type):
        """Assign segment to unknowns by comparing mass with the mass of known segments."""
        fleet_df = fleet_df.copy()
        reduced_fleet_df = fleet_df.loc[fleet_df["vehicle_type"] == vehicle_type, ["segment", "avg_mass"]]
        if vehicle_type == "car":
            ...
            mass_quantiles = mass_quantiles.drop("unknown").sort_values(ascending=False).reset_index(level=0)
        ...
        mass_quantiles = pd.IntervalIndex.from_breaks(
            mass_quantiles,
            closed="left")
        
        mapping = dict(zip(mass_quantiles, mass_labels))
        fleet_df["label"] = pd.cut(
            fleet_df["avg_mass"],
            bins=mass_quantiles).map(mapping)
        
        fleet_df["label"] = fleet_df["label"].values.add_categories("unknown").fillna("unknown")
        fleet_df.loc[(fleet_df["segment"] == "unknown") & (fleet_df["vehicle_type"] == vehicle_type), "segment"] = fleet_df["label"]
...
class CurveFitting:
    ...
    def generate_scrappage(invariant_obj):
        ...
        fleet_archive = fleet_archive.groupby(["year", "cya", "vehicle_type"])["tally"].sum().reset_index()
        fleet_archive["cya"] = fleet_archive["cya"].astype("str").str.split(",").apply(lambda x: [int(i) for i in x])
        fleet_archive["cohort_tally"] = fleet_archive["tally"] / fleet_archive["cya"].map(len)
        fleet_archive["cya_group"] = fleet_archive["cya"]
        fleet_archive["cya"] = pd.DataFrame(fleet_archive["cya"].values.tolist()).median(1)  # !!!
        ...
        fleet_archive["position_in_group"] = fleet_archive.apply(lambda x: x["cya_group"].index(x["cya"]) + 1, axis=1)
        fleet_archive["years_to_end"] = fleet_archive.apply(lambda x: len(x["cya_group"]) - x["position_in_group"], axis=1)
        
        # If it is the final year in the group set the tally as the final year tally
        fleet_archive.loc[fleet_archive["cya_group"].map(len) == 1, "final_year_tally"] = fleet_archive["cohort_tally"] 
        ...
        for i in range(5):
            fleet_archive["survival"] = fleet_archive["cohort_tally"] / fleet_archive.groupby(["cohort", "vehicle_type"])["final_year_tally"].shift(1)
            fleet_archive["survival"] = np.power(fleet_archive["survival"], 1/fleet_archive["position_in_group"])
            fleet_archive["survival"] = fleet_archive.groupby(["cya", "vehicle_type"])["survival"].transform("mean")
            fleet_archive["final_year_tally"] = fleet_archive["cohort_tally"] * np.power(fleet_archive["survival"], fleet_archive["years_to_end"])

        # Take the average of the survival rates across all cohorts
        scrappage_curve = fleet_archive.groupby(["cya", "vehicle_type"])["survival"].mean().reset_index()
    ...
    def characteristics_to_rwc(invariant_obj):
        ...
        table_df = characteristics.merge(invariant_obj.real_world_coefficients, how="inner", on=["fuel", "vehicle_type"])
    
        table_df["fc_approval"] = table_df.apply(
            "{avg_co2}*12/44 /1000/{density}*100/{carbon_ef}*1000".format_map, axis=1).map(eval)
        table_df["fc_in_use"] = table_df.apply(
            "{params1} + {params2}*{avg_es} + {params3}*{avg_mass} + {params4}*{fc_approval}".format_map, axis=1).map(eval)
        ...
        rw_multiplier = characteristics.merge(invariant_obj.fuel_characteristics, how="left", on="fuel")
        # Phevs operate in electric mode 50% of the time.
        rw_multiplier.loc[rw_multiplier["fuel_type"] == "phev", "rw_multiplier"] = rw_multiplier["rw_multiplier"] * 0.5
    ...
    def fit_se_curves(invariant_obj):
        ...
        curve_parameters = invariant_obj.naei_coefficients.merge(rw_multiplier[["fuel", "cohort", "segment"]], how="outer")
        curve_parameters = curve_parameters.loc[~(curve_parameters["fuel"] == "clean")].sort_values(by=["cohort"], ascending=False)
        curve_parameters = curve_parameters.groupby(["fuel", "segment"], as_index=False).apply(lambda group: group.ffill())
        
        # Substitute coefficients into the se curve formula
        se_curve = rw_multiplier.merge(curve_parameters, how="left", on=["cohort", "segment",  "fuel",  "vehicle_type"])
        se_curve["formula"] = se_curve.apply(
            "({alpha}*speed**2 + {beta}*speed + {gamma} + {delta}/speed) / ({epsilon}*speed**2 + {zita}*speed + {hta}) *{rw_multiplier}*{mj_co2}".format_map, axis=1) 
        
        # Set clean fuel as 0 emissions at all speeds
        se_curve.loc[se_curve["fuel"] == "clean", "formula"] = "0"
        se_curve["fuel"] = se_curve["fuel_type"]
        se_curve = se_curve[["cohort", "fuel", "vehicle_type", "segment", "formula", "min_speed", "max_speed"]]
    ...
    def calculate_speed_band_emissions(invariant_obj):
        ...
        se_calcs[speed_bands] = pd.DataFrame([[20, 40, 60, 80, 100, 120]], index=se_calcs.index)
        se_calcs = pd.melt(se_calcs,
                           id_vars=grouping_vars + ["formula", "min_speed", "max_speed"],
                           var_name="speed_band",
                           value_vars=speed_bands,
                           value_name="speed")
        se_calcs["formula"] = se_calcs.apply(lambda x: x["formula"].replace("speed", str(x["speed"])), axis=1)
        
        se_calcs["gco2/km"] = se_calcs["formula"].map(eval)
        
        # The NAEI curves are only fitted for a certain range of speeds - outside of this range they aren't accurate (e.g. high-speed HGVs)
...
class IndexFleet:
    ...
    def __init__(self, config, run_type, outpath, run_fresh=True):
        ...
        if run_fresh:        
            ...
            self.fleet_archive = pd.read_csv("{self.outpath}audit/fleet_archive.csv".format(**locals()))
            self.fleet = pd.read_csv("{self.outpath}audit/index_fleet.csv".format(**locals()))
            self.characteristics = pd.read_csv("{self.outpath}audit/characteristics.csv".format(**locals()))

    def __load_fleet(self):
        """Read in the DfT fleet data for cars, vans and HGVs and concetenate."""
        
        def read_fleet(vehicle_type, config):
            """Read in and preprocess DfT fleet data for a given vehicle type."""
            file_location = config["filePaths"][vehicle_type + "File"]
            table_df = pd.read_csv(file_location)
            table_df["VehicleType"] = vehicle_type

            # Rename columns
            table_df.columns = [i if "Euro" not in i else "EuroStandard" for i in table_df.columns]
            table_df.columns = [i if not any(z in i for z in ["MSOA", "LAD"]) else "Zone" for i in table_df.columns]
            table_df = table_df.rename(columns={"Bodytype": "Segment",
                                                "Number": "Tally"})
            return table_df

        car = read_fleet("car", self.configuration)
        lgv = read_fleet("lgv", self.configuration)
        hgv = read_fleet("hgv", self.configuration)
    ...
    def __basic_clean(self):
        ...
        fleet_archive["fuel"] = (fleet_archive["fuel"]
                                 .str.lower()
                                 .fillna("diesel"))
        
        fleet_archive["segment"] = (fleet_archive["segment"]
                                    .str.lower()
                                    .fillna("unknown")
                                    .replace({
                                        "super mini": "small",
        ...
        fleet_archive.loc[
            (fleet_archive["vehicle_type"] == "car") &      
            (fleet_archive["fuel"] == "hev"), "fuel"] = "petrol hybrid"
        # Convert hev/phev non-cars to bevs
        fleet_archive.loc[
            (fleet_archive["vehicle_type"].isin(["hgv", "lgv"])) &
            (fleet_archive["fuel"].isin(["hev", "phev"])), "fuel"] = "bev"
        ...
        # Merge vechicles with identical attributes
        self.fleet_archive = fleet_archive.groupby(ut.all_but(fleet_archive, "tally"), dropna=False).sum().reset_index()
    ...
    def __advanced_clean(self):
        ...
        """
        index_year = self.configuration["modelOptions"]["indexYear"]
        print("Index year:", index_year)
        fleet_df = self.fleet_archive.loc[self.fleet_archive["year"] == int(index_year)]
    ...
    def __map_zones(self):
        ...
        # Calculate the share each vehicle makes up of its vehicletype in its LAD
        group_by_segment = fleet_df.groupby(ut.all_but(fleet_df, "tally"))["tally"].sum().reset_index()
        group_by_segment = group_by_segment.drop(columns="year")
        group_by_segment["shr_of_lad_type"] = group_by_segment["tally"] / fleet_df.groupby(["zone", "vehicle_type"])["tally"].transform("sum")


        # Vehicle type tally by MSOA
        msoa_bodytype = pd.read_csv(self.configuration["filePaths"]["msoaBodyFile"]).fillna(0)
        msoa_bodytype = pd.melt(msoa_bodytype,
                                id_vars="MSOA11CD",
                                var_name="vehicle_type",
                                value_vars=["Cars", "LGVs", "Goods"],
                                value_name="msoa_tally")
        
        msoa_bodytype["vehicle_type"] = msoa_bodytype["vehicle_type"].replace({"Cars": "car", "LGVs": "lgv", "Goods": "hgv"})
        # Connects LAD to MSOA
        msoa_lad_lookup = pd.read_csv(self.configuration["filePaths"]["msoaLADFile"])

        if self.run_type == "MSOA":
            # join on msoa, group by msoa zones & any categories, sum by msoa zones & any categories
            msoa_zones = pd.read_csv(self.configuration["filePaths"]["msoaNoHAMFile"]).rename(columns={"msoa11cd": "MSOA11CD"})
            ...
            vehicle_msoa_zones = vehicle_msoa_zones.groupby(["MSOA11CD", "vehicle_type", "TAG_LAD"], as_index=False).sum()
            merged_data = vehicle_msoa_zones.merge(group_by_segment, left_on=["TAG_LAD", "vehicle_type"], right_on=["zone", "vehicle_type"])
            merged_data = merged_data.loc[merged_data["msoa_tally"] > 0]

            # MSOA Tally for vehicle = Vehicle share of LAD * MSOA total tally
            merged_data["tally"] = (merged_data["shr_of_lad_type"] * merged_data["msoa_tally"])
            merged_data = merged_data.groupby(["MSOA11CD", "segment", "cohort", "fuel", "vehicle_type", "msoa_tally"])["tally"].sum().reset_index()
...
class Invariant:
    ...
    def __init__(self, index_fleet_obj, config, run_type, time_period):
        ...
        config : ConfigParser obj
            Filepath lookup to import tables.
        run_type : str
            If the run_type is 'MSOA', check that the baseline demand does
        ...
        if self.configuration["auditOptions"]["saveInvariant"] == "True":
    ...
    def __export_invariants(self):
        ...
        """
        self.index_fleet.fleet_archive.to_csv("{self.index_fleet.outpath}audit/fleet_archive.csv".format(**locals()), index=False)
        self.index_fleet.fleet.to_csv("{self.index_fleet.outpath}audit/index_fleet.csv".format(**locals()), index=False)
        self.index_fleet.characteristics.to_csv("{self.index_fleet.outpath}audit/characteristics.csv".format(**locals()), index=False)
        self.scrappage_curve.to_csv("{self.index_fleet.outpath}audit/scrappage_curve.csv".format(**locals()), index=False)
    ...
    def __import_shared_tables(self):
        ...
        self.msoa_area_info = pd.read_csv(
            self.configuration["filePaths"]["msoaAreaTypeFile"])
        self.msoa_area_info = self.msoa_area_info.rename(columns={"msoaZoneID": "zone",  "R": "msoa_area_type"})
        self.new_area_types = pd.read_csv(
            self.configuration["filePaths"]["newAreaTypes"])
        self.new_area_types = self.new_area_types.rename(columns={"msoa_area_code": "zone", "tfn_area_type": "msoa_area_type"})

        # self.msoa_area_info.loc[self.msoa_area_info["fuel"] == 1|2|3|4|5, "msoa_area_type"] = 1
        # self.msoa_area_info.loc[self.msoa_area_info["fuel"] == 1, "msoa_area_type"] = 2
        # self.msoa_area_info.loc[self.msoa_area_info["fuel"] == 8, "msoa_area_type"] = 3


        if self.run_type == "MSOA":
            if self.time_period:
                 # Check that baseline demand is the same across scenarios
                car_sc01_baseline_demand_am = pd.read_csv(self.configuration["filePaths"]["DemandFile"] + "SC01/vkm_by_speed_and_type_{self.index_year}_car_AM.csv".format(**locals()))
                car_sc01_baseline_demand_ip = pd.read_csv(self.configuration["filePaths"]["DemandFile"] + "SC01/vkm_by_speed_and_type_{self.index_year}_car_IP.csv".format(**locals()))
                car_sc01_baseline_demand_pm = pd.read_csv(self.configuration["filePaths"]["DemandFile"] + "SC01/vkm_by_speed_and_type_{self.index_year}_car_PM.csv".format(**locals()))
                car_sc01_baseline_demand = pd.concat([car_sc01_baseline_demand_am, car_sc01_baseline_demand_ip, car_sc01_baseline_demand_pm])
                car_sc01_baseline_demand["Year"] = self.index_year
                car_sc01_baseline_demand = car_sc01_baseline_demand.loc[car_sc01_baseline_demand.road_type == "Motorway", "total_vehicle_km"].sum()
                for scenario_code in ["SC02", "SC03", "SC04", "SC05", "SC06"]:
                    path = self.configuration["filePaths"]["DemandFile"] + "{scenario_code}/".format(**locals())
                    car_scenario_baseline_demand_am = pd.read_csv(path + "vkm_by_speed_and_type_2018_car_AM.csv".format(**locals()))
                    car_scenario_baseline_demand_ip = pd.read_csv(path + "vkm_by_speed_and_type_2018_car_IP.csv".format(**locals()))
                    car_scenario_baseline_demand_pm = pd.read_csv(path + "vkm_by_speed_and_type_2018_car_PM.csv".format(**locals()))
                    car_scenario_baseline_demand = pd.concat([car_scenario_baseline_demand_am, car_scenario_baseline_demand_ip, car_scenario_baseline_demand_pm])
                    car_scenario_baseline_demand["Year"] = self.index_year
                    car_scenario_baseline_demand = car_scenario_baseline_demand.loc[car_scenario_baseline_demand.road_type == "Motorway", "total_vehicle_km"].sum()
                ...
                hgv_sc01_baseline_demand_am = pd.read_csv(self.configuration["filePaths"]["DemandFile"] + "SC01/vkm_by_speed_and_type_{self.index_year}_hgv_AM.csv".format(**locals()))
                hgv_sc01_baseline_demand_ip = pd.read_csv(self.configuration["filePaths"]["DemandFile"] + "SC01/vkm_by_speed_and_type_{self.index_year}_hgv_IP.csv".format(**locals()))
                hgv_sc01_baseline_demand_pm = pd.read_csv(self.configuration["filePaths"]["DemandFile"] + "SC01/vkm_by_speed_and_type_{self.index_year}_hgv_PM.csv".format(**locals()))
                hgv_sc01_baseline_demand = pd.concat([hgv_sc01_baseline_demand_am, hgv_sc01_baseline_demand_ip, hgv_sc01_baseline_demand_pm])
                hgv_sc01_baseline_demand["Year"] = self.index_year
                hgv_sc01_baseline_demand = hgv_sc01_baseline_demand.loc[hgv_sc01_baseline_demand.road_type == "Motorway", "total_vehicle_km"].sum()
                lgv_sc01_baseline_demand_am = pd.read_csv(self.configuration["filePaths"]["DemandFile"] + "SC01/vkm_by_speed_and_type_{self.index_year}_lgv_AM.csv".format(**locals()))
                lgv_sc01_baseline_demand_ip = pd.read_csv(self.configuration["filePaths"]["DemandFile"] + "SC01/vkm_by_speed_and_type_{self.index_year}_lgv_IP.csv".format(**locals()))
                lgv_sc01_baseline_demand_pm = pd.read_csv(self.configuration["filePaths"]["DemandFile"] + "SC01/vkm_by_speed_and_type_{self.index_year}_lgv_PM.csv".format(**locals()))
                lgv_sc01_baseline_demand = pd.concat([lgv_sc01_baseline_demand_am, lgv_sc01_baseline_demand_ip, lgv_sc01_baseline_demand_pm])
                lgv_sc01_baseline_demand["Year"] = self.index_year
                lgv_sc01_baseline_demand = lgv_sc01_baseline_demand.loc[lgv_sc01_baseline_demand.road_type == "Motorway", "total_vehicle_km"].sum()
                for scenario_code in ["SC02"]:
                    # hgv placeholder
                    path = self.configuration["filePaths"]["DemandFile"] + "{scenario_code}/".format(**locals())
                    hgv_scenario_baseline_demand_am = pd.read_csv(path + "vkm_by_speed_and_type_2018_hgv_AM.csv".format(**locals()))
                    hgv_scenario_baseline_demand_ip = pd.read_csv(path + "vkm_by_speed_and_type_2018_hgv_IP.csv".format(**locals()))
                    hgv_scenario_baseline_demand_pm = pd.read_csv(path + "vkm_by_speed_and_type_2018_hgv_PM.csv".format(**locals()))
                    hgv_scenario_baseline_demand = pd.concat([hgv_scenario_baseline_demand_am, hgv_scenario_baseline_demand_ip, hgv_scenario_baseline_demand_pm])
                    hgv_scenario_baseline_demand["Year"] = self.index_year
                    hgv_scenario_baseline_demand = hgv_scenario_baseline_demand.loc[hgv_scenario_baseline_demand.road_type == "Motorway", "total_vehicle_km"].sum()
                    if np.round(hgv_sc01_baseline_demand) != np.round(hgv_scenario_baseline_demand):
                        print("\n****!! HGV Baseline demand varies across scenarios!!***\n")
                    # lgv placeholder
                    path = self.configuration["filePaths"]["DemandFile"] + "{scenario_code}/".format(**locals())
                    lgv_scenario_baseline_demand_am = pd.read_csv(path + "vkm_by_speed_and_type_2018_lgv_AM.csv".format(**locals()))
                    lgv_scenario_baseline_demand_ip = pd.read_csv(path + "vkm_by_speed_and_type_2018_lgv_IP.csv".format(**locals()))
                    lgv_scenario_baseline_demand_pm = pd.read_csv(path + "vkm_by_speed_and_type_2018_lgv_PM.csv".format(**locals()))
                    lgv_scenario_baseline_demand = pd.concat([lgv_scenario_baseline_demand_am, lgv_scenario_baseline_demand_ip, lgv_scenario_baseline_demand_pm])
                    lgv_scenario_baseline_demand["Year"] = self.index_year
                    lgv_scenario_baseline_demand = lgv_scenario_baseline_demand.loc[lgv_scenario_baseline_demand.road_type == "Motorway", "total_vehicle_km"].sum()
            ...
            if not self.time_period:
                # Check that baseline demand is the same across scenarios
                car_sc01_baseline_demand = pd.read_csv(self.configuration["filePaths"]["DemandFile"] + "SC01/vkm_by_speed_and_type_{self.index_year}_car.csv".format(**locals()))
                car_sc01_baseline_demand = car_sc01_baseline_demand.loc[car_sc01_baseline_demand.road_type == "Motorway", "total_vehicle_km"].sum()
                for scenario_code in ["SC02"]:
                    path = self.configuration["filePaths"]["DemandFile"] + "{scenario_code}/".format(**locals())
                    car_scenario_baseline_demand = pd.read_csv(path + "vkm_by_speed_and_type_2018_car.csv")
                    car_scenario_baseline_demand = car_scenario_baseline_demand.loc[car_scenario_baseline_demand.road_type == "Motorway", "total_vehicle_km"].sum()
                    if np.round(car_sc01_baseline_demand) != np.round(car_scenario_baseline_demand):
                        print("\n****!! Car Baseline demand varies across scenarios!!***\n")
                # gv demand equivalents
                hgv_sc01_baseline_demand = pd.read_csv(self.configuration["filePaths"]["DemandFile"] + "SC01/vkm_by_speed_and_type_{self.index_year}_hgv.csv".format(**locals()))
                hgv_sc01_baseline_demand = hgv_sc01_baseline_demand.loc[hgv_sc01_baseline_demand.road_type == "Motorway", "total_vehicle_km"].sum()
                lgv_sc01_baseline_demand = pd.read_csv(self.configuration["filePaths"]["DemandFile"] + "SC01/vkm_by_speed_and_type_{self.index_year}_lgv.csv".format(**locals()))
                lgv_sc01_baseline_demand = lgv_sc01_baseline_demand.loc[lgv_sc01_baseline_demand.road_type == "Motorway", "total_vehicle_km"].sum()
                for scenario_code in ["SC02"]:
                    # hgv placeholder
                    path = self.configuration["filePaths"]["DemandFile"] + "{scenario_code}/".format(**locals())
                    hgv_scenario_baseline_demand = pd.read_csv(path + "vkm_by_speed_and_type_2018_hgv.csv")
                    hgv_scenario_baseline_demand = hgv_scenario_baseline_demand.loc[hgv_scenario_baseline_demand.road_type == "Motorway", "total_vehicle_km"].sum()
                    if np.round(hgv_sc01_baseline_demand) != np.round(hgv_scenario_baseline_demand):
                        print("\n****!! HGV Baseline demand varies across scenarios!!***\n")
                    # lgv placeholder
                    path = self.configuration["filePaths"]["DemandFile"] + "{scenario_code}/".format(**locals())
                    lgv_scenario_baseline_demand = pd.read_csv(path + "vkm_by_speed_and_type_2018_lgv.csv")
                    lgv_scenario_baseline_demand = lgv_scenario_baseline_demand.loc[lgv_scenario_baseline_demand.road_type == "Motorway", "total_vehicle_km"].sum()
                    if np.round(lgv_sc01_baseline_demand) != np.round(lgv_scenario_baseline_demand):
                        print("\n****!! LGV Baseline demand varies across scenarios!!***\n")
    ...
    def __warp_tables(self):
        ...
        yearly_co2_reduction["year_reduction_in_co2"] = 1 + yearly_co2_reduction["year_reduction_in_co2"]
        yearly_co2_reduction["index_carbon_reduction"] = yearly_co2_reduction.groupby("body_type")["year_reduction_in_co2"].cumprod()
        yearly_co2_reduction["cohort"] = yearly_co2_reduction["year"]
        self.yearly_co2_reduction = yearly_co2_reduction[["cohort",  "body_type", "index_carbon_reduction"]]
        
        naei_coefficients = self.naei_coefficients.copy()
        vehicle_classes = {"Passenger Cars": "car", "Light Commercial Vehicles": "lgv",  "Heavy Duty Trucks": "hgv"}
        naei_coefficients["vehicle_type"] = naei_coefficients["category"].replace(vehicle_classes)   
        ...
        naei_coefficients = naei_coefficients.drop(columns=["cya", "category", "euro_standard"])
        ...
        fleet_segments = self.index_fleet.fleet[["vehicle_type", "segment"]].drop_duplicates().copy()
        fleet_segments['bev_v_type'] = fleet_segments["vehicle_type"].replace({
            "car": "electric_car",
            "lgv": "electric_lgv",
            "hgv": "bev_hgv"})
        fleet_segments.loc[fleet_segments.segment.isin(["rigid_a_upto7.5t", "rigid_b_7.5t-14t"]), "bev_v_type"] = "bev_small_rigid"
        fleet_segments.loc[fleet_segments.segment.isin(["rigid_c_14t-20t", "rigid_d_20t-26t", "rigid_e_over26t"]), "bev_v_type"] = "bev_large_rigid"
        fleet_segments.loc[fleet_segments.segment.isin(["artic_a_upto40t", "artic_b_over40t"]), "bev_v_type"] = "bev_artic"
        ...
        grid_consumption = pd.concat([grid_consumption, 
                                      hydrogen_grid_consumption,
                                      phev_grid_consumption], 
                                     ignore_index=True)
        self.grid_consumption = grid_consumption.copy()
        
        # Index grid CO2 intensity figures from a specified scenario
        grid_scenario = {"CCC Balanced": "ccc_6_cb", "TAG": "tag"}[self.grid_intensity_scenario]
    ...
    def __load_anpr(self):
        ...
        for i in ["Car", "Van", "HGV"]:
            sheet = i + " output"
            # Import relevant tab
            anpr_info = pd.read_excel(
                self.configuration["filePaths"]["anprFile"],
                sheet_name=sheet)
            ...
            anpr_info["body_type"] = anpr_info["body_type"].replace({"HGV - Rigid": "Rigid", "HGV - Artic": "Artic"})
            anpr_info["vehicle_type"] = i
            # Normalise the data so that the groups add up to 100 percent
            anpr_info["percentage"] = anpr_info["percentage"] / anpr_info.groupby(["fuel",  "vehicle_type",  "road_type"])["percentage"].transform("sum")
            ...
            if i == "Car":
                ...
                petrol_diesel_share["cya"] = petrol_diesel_share["year"] - petrol_diesel_share["cohort"]
                ...
                petrol_diesel_share = petrol_diesel_share.loc[petrol_diesel_share.fuel.isin(["petrol", "diesel"])].groupby(["body_type", "cya", "fuel"])["tally"].sum().reset_index()
                petrol_diesel_share["petrol_diesel_prop"] = petrol_diesel_share["tally"] / petrol_diesel_share.groupby(["body_type", "cya"])["tally"].transform("sum")
                petrol_diesel_share = petrol_diesel_share.drop(columns="tally")
                
                # Merge with the anpr data and derive the weighted average
                anpr_info = anpr_info.merge(petrol_diesel_share, on=["body_type", "cya", "fuel"], how="left")
                anpr_info["percentage"] = anpr_info["percentage"] * anpr_info["petrol_diesel_prop"]
                anpr_info = anpr_info.groupby(["road_type", "body_type", "cya"])["percentage"].sum().reset_index()

            # For HGVs, simply find the mean percentage for vehicle type (Artic and Rigid combined)
            elif i == "HGV":
                anpr_info["percentage"] = anpr_info.groupby(["road_type", "vehicle_type", "cya"])["percentage"].transform("mean")
                anpr_info = anpr_info[["road_type", "body_type", "cya", "percentage"]]

            # For vans and HGVs, simply find the mean percentage
            else:
                anpr_info = anpr_info.groupby(["road_type", "body_type", "cya"])["percentage"].mean().reset_index()
                
            # Re-normalise the percentages
            anpr_info["vehicle_type"] = i
            anpr_info["percentage"] = anpr_info["percentage"] / anpr_info.groupby(["vehicle_type", "road_type"])[
        ...
        anpr["vehicle_type"] = anpr["body_type"].replace(
            {"Car":  "car",
             ...
             "Rigid": "hgv",
             "Artic": "hgv"})
        anpr = anpr.rename(columns={"percentage": "cya_prop_of_bt_rt"})   # Fuel Body Type and Road Type
        
        self.anpr = anpr[["body_type", "road_type", "vehicle_type", "cya", "cya_prop_of_bt_rt"]]
