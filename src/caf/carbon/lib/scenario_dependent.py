...
class Scenario:
    ...
    def __init__(self, config, run_type, filter, time_period, time, scenario_name, invariant_obj, pathway="none"):
        ...
        run_type : str
            If the run_type is 'NELUM' or 'NoHAM', act accordingly
        time_period : bool
    ...
    def __load_scenario(self, pathway="none"):
        ...
        if self.index_year == 2015:   # !
            seg_share_2015 = pd.read_csv("CAFCarb/input/seg_prop_vtype_2015.csv")
            seg_share_2015.columns = ["segment", 2015]
            self.seg_share_of_year_type_sales = self.seg_share_of_year_type_sales.merge(seg_share_2015, 
                                                                                        on="segment", 
                                                                                        how="left")  
            
            fuel_share_2015 = pd.read_csv("NoCarb/input/fuel_prop_segment_2015.csv")
    ...
    def __merge_demand(self):
        ...
        self.demand = self.demand.loc[self.demand['zone'].isin(self.filter["msoa11_id"])].reset_index(drop=True)

    def __load_car_demand(self):
        if self.run_type == "MSOA":
            if self.time_period:
                path = self.configuration["filePaths"]["DemandFile"] + "{}/".format(self.scenario_code)
    ...
    def __load_demand(self, vehicle_type):
        ...
        if self.run_type == "MSOA":
            ...
            # this column only relevant for unmodelled hgv/lgv growth
            demand["km_growth"] = 1
        ...
        demand = pd.melt(demand, 
                         id_vars=["zone", "road_type", "total_vehicle_km", "vehicle_type", "year", "km_growth"],
                         var_name="speed_band",
                         value_name="chainage")
