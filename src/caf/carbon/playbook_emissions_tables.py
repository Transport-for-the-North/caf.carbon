...
class PlaybookProcess:
    ...
    def read_cafcarb_output(self, year, scenario):
        ...
        through_lta = self.code_lookup[["lad", "through_name"]].rename(columns={"lad": "through"})
        ...
        year_data = (
            year_data.groupby(
                ["origin", "destination", "through", "fuel", "user_class", "vehicle_type",
                 "year", "trip_band", "time_period", "tally"]
    ...
    def create_trip_length(self, annual_data):
        trip_data = annual_data[["origin", "destination", "through", "trip_band", "tailpipe_gco2", "grid_gco2"]]
    ...
    def create_genesis(self, annual_data):
        genesis_data = annual_data[["origin", "destination", "through", "trip_band", "tailpipe_gco2", "grid_gco2"]]
        origin_lta = self.code_lookup[["zone_cd", "through_name"]].rename(columns={"zone_cd": "origin",
                                                                                   "through_name": "o_lta"})
        genesis_data = genesis_data.merge(origin_lta, how="left", on="origin")
        destination_lta = self.code_lookup[["zone", "through_name"]].rename(columns={"zone": "destination",
                                                                                   "through_name": "d_lta"})
    ...
    def vkm_info(self, data):
        data = data[["through", "year", "tailpipe_gco2", "grid_gco2"]].groupby(
            ["through", "year"], as_index=False).sum()
        data_totals = data[["through", "year", "vehicle_type", "vkm"]].groupby(
        ...
        data = data[["LTA", "year", "zev mileage", "vehicle_type"]].groupby(
            ["LTA", "year", "vehicle_type"], as_index=False).sum()
        transformed_data = data.pivot(index=["LTA", "year"], columns="vehicle_type", values="zev_mileage")
        return transformed_data

    def co2_info(self, data):
        data = data[["through", "year", "tailpipe_gco2", "grid_gco2"]].groupby(
            ["through", "year", "vehicle_type", "fuel_type"], as_index=False).sum()
        data["Emissions (MtCO2e)"] = data["tailpipe_gco2"] + data["grid_gco2"]
        data["Emissions (MtCO2e)"] = data["Emissions (MtCO2e)"] / (1000 * 1000 * 1000 * 1000)
        data = data.rename(columns={"through": "LTA"})
        data = data[["LTA", "year", "Emissions (MtCO2e)"]].groupby(
            ["LTA", "year"], as_index=False).sum()
        return data
    ...
    def run_analysis(self):
        ...
        emissions_2019 = self.lininterpol(emissions_2018, emissions_2028, 2018, 2028, 2019,
                                          ["tailpipe_gco2", "grid_gco2", "vkm"])
        ...
        emissions_2050 = self.lininterpol(emissions_2043, emissions_2048, 2043, 2048, 2050,
                                          ["tailpipe_gco2", "grid_gco2", "vkm"])

        vehicle_2050 = self.create_vehicle_type(emissions_2050)
        vehicle_2050.to_csv("2050 Vehicle.csv")

        vkm_splits_bau = self.create_vkm_splits(emissions_2018,
                                            emissions_2028,
                                            emissions_2038,
                                            emissions_2043,
                                            emissions_2048)
        ...
        vkm_splits_ue = self.create_vkm_splits(ue_emissions_2018,
                                                ue_emissions_2028,
                                                ue_emissions_2038,
                                                ue_emissions_2043,
                                                ue_emissions_2048)
