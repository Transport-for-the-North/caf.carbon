...
class Demand:
    ...
    def __init__(self, available_years):
        ...
        self.car_demand = pd.DataFrame(columns=["origin_zone", "vkm_10-30_kph", "vkm_30-50_kph",
                                               "vkm_50-70_kph", "vkm_70-90_kph", "vkm_90-110_kph", "total_vehicle_km", "year"])
        self.lgv_demand = pd.DataFrame(columns=["origin_zone", "vkm_10-30_kph", "vkm_30-50_kph",
                                               "vkm_50-70_kph", "vkm_70-90_kph", "vkm_90-110_kph", "total_vehicle_km", "year"])
        self.hgv_demand = pd.DataFrame(columns=["origin_zone", "vkm_10-30_kph", "vkm_30-50_kph",
                                               "vkm_50-70_kph", "vkm_70-90_kph", "vkm_90-110_kph", "total_vehicle_km", "year"])
        self.process_demand()
    ...
    def assign_speed_band_vkm(self, link_data, demand_data, user_class, year, component):
        ...
        if demand_data.empty == False:
            ...
            demand_data.loc[((demand_data[user_class_speed] >= 30) & (
                    demand_data[user_class_speed] < 50)), "speed_band"] = "vkm_30-50_kph"
            demand_data.loc[((demand_data[user_class_speed] >= 50) & (
                        demand_data[user_class_speed] < 70)), "speed_band"] = "vkm_50-70_kph"
    ...
    def linear_interpol(self, vehicle_type, year1, year2, target_year):
        ...
        for i in ["vkm_10-30_kph", "vkm_30-50_kph", "vkm_50-70_kph", "vkm_70-90_kph",
                  "vkm_90-110_kph", "total_vehicle_km"]:
            a = df2year - year
            b = year - df1year
    ...
    def process_demand(self):
        ...
        for year in self.available_years:
            for time in time_period:
                if year == 2019:
                    network_links = pd.read_csv(self.link_data_path + fr"\{year}\link_table_SE_B19_{time}_net_v020.csv").rename(columns={"A": "a", "B": "b"})
                else:
                    network_links = pd.read_csv(self.link_data_path + fr"\{year}\link_table_SE_DM_FY{year}_{time}_net_v002.csv").rename(columns={"A": "a", "B": "b"})
                ...
                for user_class in user_classes:
                    ...
                    if year == 2019:
                        demand_table = pd.DataFrame(pd.read_hdf(self.demand_data_path + fr"\{year}\SE_B19_{time}_net_v020_SatPig_{user_class}.h5")).reset_index()
                    else:
                    ...
                    for component in batch(range(1, demand_max + 1), 1000):
                        ...
                        speed_data = self.assign_speed_band_vkm(network_links, demand_component, user_class, year, component)
